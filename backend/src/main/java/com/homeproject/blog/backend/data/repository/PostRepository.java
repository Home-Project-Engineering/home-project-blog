...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query(value = "SELECT post FROM PostEntity post LEFT JOIN post.tags tags WHERE (:id is null or post.id = :id) and (:tag_id is null or tags.id = :tag_id) and (:tag_name is null or tags.name = :tag_name) and (:author_name is null or post.author.name = :author_name)")
